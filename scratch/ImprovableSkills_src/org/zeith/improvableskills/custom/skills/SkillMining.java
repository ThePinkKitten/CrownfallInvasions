...
implements IDigSpeedAffectorSkill {
    public SkillMining() {
        super(25);
        this.xpCalculator.setBaseFormula("sqrt(%lvl%^3)*3");
    }
    ...
    public float getDigMultiplier(ItemStack stack, BlockPos pos, PlayerSkillData data) {
        ...
        if (b.canHarvestBlock((BlockGetter)data.player.m_9236_(), pos, data.player) && b.m_204336_(BlockTags.f_144282_) && data.player.m_21205_().m_41691_(b) > 0.0f) {
            return (float)data.getSkillLevel(this) / 8.0f;
        }
        return 0.0f;
    }
}
